...
import 'package:papilus_component_gramedia/v2/components/segmented/segmenteds.dart';
...
/// * `height` - height panel
class PapyrusSegmentedWidget<T> extends StatefulWidget {
  const PapyrusSegmentedWidget({
    super.key,
  ...
  final double? height;
  final PapyrusSegmentedController<T>? controller;
  final SegmentedSettings? customSegmentSettings;

  @override
  @override
  State<PapyrusSegmentedWidget> createState() => _PapyrusSegmentedWidgetState();
}

class _PapyrusSegmentedWidgetState<T> extends State<PapyrusSegmentedWidget<T>> {
  T? current;
  double? height;
  double offset = 0.0;
  Map<T?, double> sizes = {};
  ...
  @override
  void didUpdateWidget(covariant PapyrusSegmentedWidget<T> oldWidget) {
  ...
  Widget _dividerItem(int index, MapEntry<T?, double> item) {
    final currentIndex = widget.children.keys.toList().indexOf(current as T);
    int? prevIndex;
    if (currentIndex > 0) {
